...
type SessionLogEntry =
    | { kind: 'message'; key: string; message: RawJSONLines; cwd?: string }
        ...
        for (let session of sessions) {
            ...
            for (let entry of sessionEntries) {
                ...
                if (entry.kind === 'message') {
                    ...
                    // on each transcript record but fires no hook. Detect it here, but
                    // only follow when it looks like a real session relocation — a
                    // worktree root, or a return to the launch dir (worktree exited) —
                    // NOT the transient subdir `cd`s a build/typecheck performs.
                    const recordCwd = entry.cwd;
                    if (
                        recordCwd
                        && recordCwd !== lastCwd
...
async function readSessionEntries(projectDir: string, sessionId: string): Promise<SessionLogEntry[]> {
    ...
    for (let l of lines) {
        try {
            ...
            const recordCwd = (message as { cwd?: unknown }).cwd;
            entries.push({
                kind: 'message',
                key: messageKey(parsed.data),
                message: parsed.data,
                cwd: typeof recordCwd === 'string' && recordCwd.length > 0 ? recordCwd : undefined,
            });
        } catch (e) {
