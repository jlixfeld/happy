...
describe('PermissionHandler', () => {
    it('auto-approves tool calls in yolo mode without surfacing a request', async () => {
        ...
        const result = await handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: controller.signal, toolUseID: 'toolu_yolo', requestId: 'toolu_yolo' },
        );
    ...
    it('auto-approves tool calls in bypassPermissions mode', async () => {
        ...
        const result = await handler.handleToolCall(
            ...
            mode,
            { signal: controller.signal, toolUseID: 'toolu_bypass', requestId: 'toolu_bypass' },
        );
    ...
    it('keeps main-thread request IDs unchanged', async () => {
        ...
        const pending = handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: controller.signal, toolUseID: 'toolu_main', requestId: 'toolu_main' },
        );
    ...
    it('uses agentID to disambiguate sub-agent permission requests with the same toolUseID', async () => {
        ...
        const firstPending = handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: firstController.signal, toolUseID: 'toolu_shared', agentID: 'agent-a', requestId: 'toolu_shared' },
        );
        const secondPending = handler.handleToolCall(
            'Bash',
            { command: 'whoami' },
            mode,
            { signal: secondController.signal, toolUseID: 'toolu_shared', agentID: 'agent-b', requestId: 'toolu_shared' },
        );
    ...
    it('can look up a single sub-agent response by raw toolUseID for transcript follow-up paths', async () => {
        ...
        const pending = handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: controller.signal, toolUseID: 'toolu_result', agentID: 'agent-a', requestId: 'toolu_result' },
        );
...
describe('PermissionHandler.handleModeChange', () => {
    // `auto` is implemented entirely by the binary's own classifier —
    // handleToolCall has no `auto` branch — so a mode change to `auto` MUST be
    // pushed to the live query or it silently degrades to `default` and prompts
    // for everything.
    it('pushes `auto` to the live query so the binary classifier is armed', () => {
        const { session } = createSessionMock();
        const handler = new PermissionHandler(session as any);
